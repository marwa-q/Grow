        ...
        <form action="{{ route('donations.update', $donation) }}" method="POST">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('activity_id') is-invalid @enderror" id="activity_id" name="activity_id" required>
                        ...
                        <option value="{{ $activity->id }}" {{ old('activity_id', $donation->activity_id) == $activity->id ? 'selected' : '' }}>
                            {{ $activity->title }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" min="1" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $donation->amount) }}" required>
                    </div>
                    @error('amount')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('transaction_id') is-invalid @enderror" id="transaction_id" name="transaction_id" value="{{ old('transaction_id', $donation->transaction_id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control @error('donated_at') is-invalid @enderror" id="donated_at" name="donated_at" value="{{ old('donated_at', $donation->donated_at ? $donation->donated_at->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}" required>
                @error('donated_at')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
